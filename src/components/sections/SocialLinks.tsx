...
import type { SocialItem } from "@/data/socialLinks";
import { socialItems } from "@/data/socialLinks";

type SocialIconVariant = SocialItem["icon"];

function SocialIcon({ variant }: { variant: SocialIconVariant }) {
  const classes =
    variant === "github"
  ...
  return (
    <div
      ...
      className={`w-14 h-14 ${classes} rounded-xl flex items-center justify-center shadow-lg cursor-pointer transform transition-all duration-300 ease-out`}
    >
      {variant === "github" ? (
        <svg
          viewBox="0 0 24 24"
...
function SocialLinkItem({ item }: { item: SocialItem }) {
  return (
    <div className="h-full x-border p-2">
      <div className="flex items-center space-x-2">
        <div className="relative">
          <SocialIcon variant={item.icon} />
        </div>
